...
const Modal: React.FC<{
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-2xl p-6 md:p-8 w-full max-w-lg relative max-h-full overflow-y-auto"
...
const TransactionForm: React.FC<{
  ...
  const [isRecurring, setIsRecurring] = useState<boolean>(
    transactionToEdit?.isRecurring || false
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      type,
      amount: parseFloat(amount), // ✅ numeric conversion
      category,
      date,
      description,
      isRecurring,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end pt-4 space-x-2">
        ...
        >
          {transactionToEdit ? 'Update' : 'Save'} Transaction
        </button>
      </div>
    </form>
  );
    ...
    <div>
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow transition-colors w-full md:w-auto"
        >
      ...
      <div className="bg-white p-4 rounded-xl shadow-md mb-6 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
        ...
        />
        <Dropdown options={typeFilterOptions} value={filterType} onChange={setFilterType} />
        <Dropdown options={categoryFilterOptions} value={filterCategory} onChange={setFilterCategory} />
      </div>
        ...
        {filteredTransactions.map((t) => (
          <div key={t.id} className="bg-white rounded-xl shadow-md p-4">
            ...
            <div className="flex justify-end items-center mt-2 border-t pt-2">
              <span
                className={`px-2 py-1 text-xs font-semibold rounded-full mr-auto ${
                  t.type === TransactionType.INCOME
                    ? 'bg-green-100 text-green-800'
                    : 'bg-red-100 text-red-800'
                }`}
