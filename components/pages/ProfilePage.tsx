...
const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  // ✅ Compute stats safely with numeric conversion
  const accountStats = useMemo(() => {
    ...
    if (expenseTransactions.length > 0) {
      ...
      if (entries.length > 0) {
        // Coerce to Number to avoid TS arithmetic type complaints and guard empty entries
        mostFrequentCategory = entries.sort((a, b) => Number(b[1]) - Number(a[1]))[0][0];
      } else {
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onload = (event: ProgressEvent<FileReader>) => {
        ...
        if (result) setCurrentAvatar(result as string);
      };
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-md text-center">
              ...
              <label
                ...
                className="cursor-pointer px-5 py-2.5 bg-gray-200 text-gray-800 font-medium rounded-lg hover:bg-gray-300 transition-colors"
              >
                Upload Image
              </label>
            </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-md">
              ...
              <div className="flex justify-end pt-8">
                <button
                  type="submit"
                  className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 shadow transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  Save Changes
                </button>
              </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-md">
              ...
              <div className="space-y-1">
                ...
                <StatItem
                  ...
                  value={`${currency.symbol}${accountStats.totalIncome.toFixed(2)}`}
                />
                <StatItem
                  label="Lifetime Expenses"
                  value={`${currency.symbol}${accountStats.totalExpenses.toFixed(2)}`}
                />
                <StatItem
                  label="Top Expense Category"
                  value={accountStats.mostFrequentCategory}
                />
              </div>
            </div>
          </div>
